...
Custom IoU Tracking, ID Persistence, and Vocal Interaction
This member focused on developing the lightweight, custom Intersection over Union (IoU) tracker to handle bounding box data efficiently. A major part of their work involved solving ID flickering by implementing Exponential Moving Average (EMA) smoothing and significantly extending the ID retention timeout so a person who leaves the frame is re-identified correctly. Furthermore, they integrated the text-to-speech (TTS) fallback system for robotic voice interaction.

Files Allotted:
updatedDetector.py (Partial): Focus on the Track and IoUTracker classes, including the matrix-based argmax matching logic and EMA smoothing parameters. Also responsible for the pyttsx3 initialization and the audio trigger ("Are you okay? Please respond.") during the FALL_CONFIRMED event.
